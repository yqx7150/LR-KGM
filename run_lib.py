...
def train(config, workdir):
  ...
  checkpoint_dir = os.path.join(workdir, "checkpoints_noHankel")
  # Intermediate checkpoints to resume training after pre-emption in cloud environments
  checkpoint_meta_dir = os.path.join(workdir, "checkpoints-meta_noHankel", "checkpoint.pth")
...
#    batch = torch.from_numpy(inps).to(config.device).float()
    ...
    if step != 0 and step % config.training.snapshot_freq == 0 or step == num_train_steps:
      ...
      if config.training.snapshot_sampling:
        ...
        kw_real, kw_imag = np.real(sample[0,:,:,0]), np.imag(sample[0,:,:,0])  # 原来126
